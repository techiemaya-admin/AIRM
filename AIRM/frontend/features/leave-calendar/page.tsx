    ...
    <div className="min-h-screen bg-white">
                ...
                <div className="rounded-2xl bg-white shadow-sm border border-gray-200 mb-6 overflow-hidden">
                  ...
                  <div className="flex flex-wrap items-center justify-between px-6 py-5 gap-4">
                    ...
                    <div className="flex flex-wrap items-center gap-3 mt-2 sm:mt-0 flex-shrink-0">
                      <button onClick={handlePrevMonth} className="whitespace-nowrap flex-shrink-0 px-4 py-2 text-sm font-medium text-gray-700 border border-gray-300 rounded-lg bg-white hover:bg-gray-50 transition shadow-sm">Previous</button>
                      <button onClick={handleToday} className="whitespace-nowrap flex-shrink-0 px-4 py-2 text-sm font-medium text-gray-700 border border-gray-300 rounded-lg bg-white hover:bg-gray-50 transition shadow-sm">Today</button>
                      <button onClick={handleNextMonth} className="whitespace-nowrap flex-shrink-0 px-4 py-2 text-sm font-medium text-gray-700 border border-gray-300 rounded-lg bg-white hover:bg-gray-50 transition shadow-sm">Next</button>
                    </div>
                  </div>
                ...
                <div className="space-y-6">
                  ...
                  <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
                    <div className="px-6 py-5 flex items-center justify-between border-b border-gray-100">
                      ...
                      {isAdmin && (
                        <Button onClick={() => { setBalanceUserId(selectedUserId !== 'all' ? selectedUserId : ''); setShowSetBalanceDialog(true); }} className="bg-[#1E3A8A] hover:bg-[#1E3A8A]/90 text-white text-sm font-semibold rounded-lg px-4 py-2 h-10">
                    ...
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        {leaveBalances.length === 0 ? (
                          <tr>
                            <td colSpan={5} className="text-center py-10 text-gray-400 text-sm italic">No balance records found.</td>
                          </tr>
                ...
                try {
                  ...
                  const csvRows = sortedAttendance.map(rec => {
                    ...
                    return [
                      ...
                      `"${(Number(rec.total_hours) || 0).toFixed(2)}"`
                    ].join(',');
                  });
                  ...
                  link.setAttribute('download', `Attendance_${safeUserName}_${format(attendanceDate, 'MMM_yyyy')}.csv`);
                    ...
                    <div className="flex flex-wrap items-center justify-between px-6 py-5 gap-4">
                      ...
                      <div className="flex flex-wrap items-center gap-3 mt-2 sm:mt-0 flex-shrink-0">
                        ...
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => attendanceView === 'week' ? setAttendanceDate(d => subWeeks(d, 1)) : setAttendanceDate(d => subMonths(d, 1))}
                          className="bg-white text-gray-700"
                        >
                          Previous {attendanceView === 'week' ? 'Week' : 'Month'}
                        </Button>
                        <Button
                          variant="outline"
                          size="sm"
                          ...
                          {filteredAttendance.length === 0 ? (
                            ...
                            return (
                              <tr key={i} className="hover:bg-gray-50 transition">
                                ...
                                <td className="px-6 py-4 text-gray-700">{(Number(rec.total_hours) || 0).toFixed(1)}</td>
                              </tr>
                            );
                          })}
                        </tbody>
        ...
        <DialogContent>
          ...
          <div className="space-y-4 py-2">
            ...
            <div>
              ...
              <Input type="number" step="0.5" min="0" value={balanceAmount} onChange={e => setBalanceAmount(e.target.value)} placeholder="e.g. 12" className="mt-1" />
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setShowSetBalanceDialog(false)}>Cancel</Button>
            <Button
              ...
              onClick={() => {
                ...
                }, { onSuccess: () => { setShowSetBalanceDialog(false); setBalanceAmount(''); } });
              }}
            >Save Balance</Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog open={showAssignShiftDialog} onOpenChange={setShowAssignShiftDialog}>
        <DialogContent>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setShowAssignShiftDialog(false)}>Cancel</Button>
            <Button
