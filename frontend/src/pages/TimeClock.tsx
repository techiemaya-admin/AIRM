...
const TimeClock = () => {
  ...
  const loadCurrentEntry = async () => {
    try {
      ...
      logger.error("Error loading current entry:", error);
      setCurrentEntry(null);
    }
  };
  ...
  const getAddressFromCoordinates = async (latitude: number, longitude: number): Promise<string | null> => {
    try {
      ...
      if (!response.ok) {
        logger.warn("Failed to fetch address");
        return null;
      }
      ...
      logger.warn("Error fetching address:", error);
      return null;
    }
  };
  ...
  const performClockOut = async (comment?: string) => {
    ...
    try {
      ...
      toast({
        ...
        description: `${formatHours(totalHours)} saved to timesheet${comment ? ' with comment' : ''}. Timesheet will refresh automatically.`,
        duration: 5000,
      });
                ...
                <div className={`p-4 rounded-lg border ${currentEntry.status === 'paused'
                  ...
                  {currentEntry.issue?.estimated_hours !== undefined && (
                    <p className="text-sm font-medium text-blue-600 dark:text-blue-400">
                      Estimate: {formatHours(parseFloat(currentEntry.issue.estimated_hours as any))}
                    </p>
                  )}
                  <p className="text-sm text-muted-foreground">
                  ...
                  {currentEntry.paused_duration && currentEntry.paused_duration > 0 && (
                    <p className="text-sm text-muted-foreground">
                      Paused time: {formatHours(currentEntry.paused_duration)}
                    </p>
                  )}
                  {currentEntry.notes && (
