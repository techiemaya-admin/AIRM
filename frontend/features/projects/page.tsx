...
type WizardStep = 'list' | 'repository' | 'project' | 'templates' | 'confirm';
...
const Projects = ({ onProjectSelect, onlyProjects = false }: { onProjectSelect?: (projectName: string) => void, onlyProjects?: boolean }) => {
  ...
  const openProjectBoard = async (project: any) => {
    ...
    if (project.source === 'github_project' || String(project.id).startsWith('PVT_')) {
      ...
      try {
        const ghProjects: any[] = await api.git.getProjects() as any[];
  ...
  const handleAddItem = async (title: string, status: string = 'Todo') => {
    ...
    const githubNodeId = (selectedBoard as any).githubNodeId;
    if (githubNodeId) {
      try {
  ...
  const fetchRepoData = async () => {
    ...
    try {
      ...
      try {
        const prs = await (api.git as any).getRepoPullRequests(owner, repoName2);
  ...
  const toggleLabel = async (label: string) => {
    ...
    try {
      // Sync with GitHub
      await (api.git as any).updateIssue(
    ...
    <div className="container mx-auto p-6 space-y-6">
      ...
      <div className="space-y-6">
        {onlyProjects ? (
          ...
          <>
            {/* Semantic wrapper (hidden visually but present for accessibility) */}
                    ...
                    {loadingGhProjects ? (
                      ...
                      <div className="space-y-2 max-h-[300px] overflow-y-auto pr-2">
                        {ghProjects.map(p => (
                          ...
                          >
                            <span className="text-sm font-medium">{p.name || p.title}</span>
