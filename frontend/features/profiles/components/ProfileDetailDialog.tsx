...
export const ProfileDetailDialog = ({
  ...
  const handleExitEmployee = async () => {
    ...
    if (!window.confirm(`Are you sure you want to mark ${profile.full_name || profile.email} as an Ex-Employee? Their details will be retained.`)) {
      return;
    }

    try {
      await api.users.updateRole(profile.id, 'ex-employee');
